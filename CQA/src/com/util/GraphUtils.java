...
public class GraphUtils {
	...
	public static <T> void conTraversal(Map<T,List<T>> graph,T element,List<T> Visited) { // Traversing the map using dfs
		if(Visited.indexOf(element)==-1)			// If the element is not visited add it to the visited array
		{
			...
			for(int i = 0;i<t;i++)
			{
				conTraversal(graph,graph.get(element).get(i),Visited);	// Traverse through the vertices connected to these edges
			}
		}
	}
	
	// Overloaded functions for directed and undirected graph to check if graph is cyclic
	...
	public static <T> boolean isConnected(Map<T,List<T>> graph) {
		Map<T,List<T>> temp = graph.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey,Map.Entry::getValue));
